...
jest.mock('glob', () => ({
  sync: () => ['project-name/path-1', 'project-name/path-2'],
}));
...
describe('Stats', () => {
  ...
  beforeEach(() => {
    myEmitter = {
      emit: (event, filesPath) =>
        `emit an ${event} event with this ${filesPath}`,
    };
  ...
  it('should invoke myEmitter correctly', () => {
    ...
    expect(myStats).toEqual(
      'emit an event event with this project-name/path-1,project-name/path-2',
    );
  });
